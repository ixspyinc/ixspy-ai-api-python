...
- 同步客户端，基于 `requests`；运行时依赖为 `requests>=2.25`、`urllib3>=1.26`、
  `packaging>=20`（后两者是 `requests` 的传递依赖与 PEP 440 版本解析所需，见
  「版本号」一节）。
...
缺少 `Version` 字段时继续查询已安装元数据。

构建期则是严格模式，对 sdist 与仓库检出区别对待：

| 构建来源 | 行为 |
| --- | --- |
| sdist（源码树带本发行版的 `PKG-INFO`） | 版本必须来自环境变量、Git 标签或该 `PKG-INFO`；`PKG-INFO` 存在但版本缺失/非法时**直接报错**，绝不改用构建机上已安装的版本号 |
| 仓库检出（无 `PKG-INFO`，如 `pip install .` 的隔离构建拷贝） | 无"这份源码的版本"可用，允许沿用已安装版本；未安装则回退 `0.0.0` |

这样做是为了避免把构建机上的旧版本号打进新的发行包：从 sdist 重建时若 `PKG-INFO`
缺少版本，采用本机已安装的版本会静默产出错误版本号的包，因此宁可失败。发布流程
依赖 Git 标签或 `IXSPY_AI_API_VERSION` 提供真实版本。
